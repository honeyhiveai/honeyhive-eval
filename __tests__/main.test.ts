...
describe('GitHub Actions Script', () => {
  ...
  it('should set outputs correctly on a successful API call', async () => {
    ...
    expect(setOutputMock).toHaveBeenCalledWith('passed', ['datapoint1', 'datapoint2']);
    expect(setOutputMock).toHaveBeenCalledWith('failed', []);
    expect(setOutputMock).toHaveBeenCalledWith('metrics', mockApiResponse.metrics);
    expect(setOutputMock).toHaveBeenCalledWith('datapoints', mockApiResponse.datapoints);
  });
  ...
  it('should fail the workflow if the API request returns a non-200 status', async () => {
    ...
    mockGet.mockResolvedValue({
      ...
      readBody: jest.fn().mockResolvedValue(JSON.stringify({ error: 'Server error' })),
    });

    await run();

    // Ensure core.setFailed was called with the appropriate error message
    expect(setFailedMock).toHaveBeenCalledWith('API request failed with status code 500');
  });
  ...
  it('should handle and report an error from the API response', async () => {
    ...
    expect(setFailedMock).toHaveBeenCalledWith(expect.stringContaining('Unexpected token'));
  });
});
